...
const _jsonToVideoId = (json) => {
  ...
  return (
    tabs &&
    tabs.filter((tab) => tab.tabRenderer && tab.tabRenderer.content)[0]
...
const _jsonToProfile = (json) => {
  ...
  const photo =
    json.microformat.microformatDataRenderer.thumbnail.thumbnails[0].url;
  return { owner, photo };
};
...
const GetDataChannel = async (channel: string): Promise<unknown> => {
  ...
  const videos = await Promise.all(
    items.map(
      async ({ gridVideoRenderer }) => await GetVideo(gridVideoRenderer.videoId)
    )
  );
